...
class JAnalytics {
  ...
  ///
  /// [period] 最小10秒，最大1天，超出范围会打印调用失败日志。传0表示统计数据即时上报
  static Future<void> setReportPeriod(Duration period) async {
    assert(
      period == Duration.zero ||
          (period.inSeconds >= 10 && period.inSeconds <= 24 * 60 * 60),
    );
    await platform(
      android: (pool) async {
        final context = await android_app_Application.get();
      ...
      ios: (pool) async {
        await JANALYTICSService.setFrequency(period.inSeconds);
      },
    );
  }
